...
# Creacion de la denseNet, codigo copido del github
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ("conv0", nn.Conv2d(1, channels, (7, 7), (2, 2), (3, 3), bias=False)),
...
def densenet121(**kwargs: Any) -> DenseNet:
    model = DenseNet((6, 12, 24, 16), 1, 32, **kwargs)

    return model


def densenet161(**kwargs: Any) -> DenseNet:
    model = DenseNet((6, 12, 36, 24), 1, 48, **kwargs)

    return model


def densenet169(**kwargs: Any) -> DenseNet:
    model = DenseNet((6, 12, 32, 32), 1, 32, **kwargs)

    return model


def densenet201(**kwargs: Any) -> DenseNet:
    model = DenseNet((6, 12, 48, 32), 1, 32, **kwargs)
