...
from models.kans.kans import RadialBasisFunction
...
class FastKANConvNDLayer(nn.Module):
    ...
    def forward_fast_kan(self, x, group_index):
        ...
        if spline_basis.dim() == 3:
            spline_basis = spline_basis.permute(0, 2, 1).flatten(1, 2)
        elif spline_basis.dim() == 4:
            spline_basis = spline_basis.permute(0, 2, 3, 1).flatten(1, 3)
        elif spline_basis.dim() == 5:
            spline_basis = spline_basis.permute(0, 2, 3, 4, 1).flatten(1, 4)
        # Reshape spline_basis to match the expected input dimensions for the convolutional layer
        if self.ndim == 1:
            spline_basis = spline_basis.view(spline_basis.size(0), self.grid_size * self.inputdim // self.groups, -1)
        elif self.ndim == 2:
            spline_basis = spline_basis.view(spline_basis.size(0), self.grid_size * self.inputdim // self.groups, *x.shape[2:])
        elif self.ndim == 3:
            spline_basis = spline_basis.view(spline_basis.size(0), self.grid_size * self.inputdim // self.groups, self.kernel_size, self.kernel_size, self.kernel_size)
